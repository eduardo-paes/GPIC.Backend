...
namespace Adapters.Services
{
    public class AreaService : IAreaService
    {
        ...
        public async Task<ResponseDTO> Create(RequestDTO model)
        {
            ...
            return _mapper.Map<ResponseDTO>(result);
        }
        ...
        public async Task<IQueryable<ResponseDTO>> GetAreasByMainArea(Guid? mainAreaId, int skip, int take)
        {
            var result = await _getAreasByMainArea.Execute(mainAreaId, skip, take);
            return _mapper.Map<IQueryable<ResponseDTO>>(result);
        }
